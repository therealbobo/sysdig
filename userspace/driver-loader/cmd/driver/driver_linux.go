...
import (
	...
	drivercleanup "github.com/draios/sysdig/userspace/driver-loader/cmd/driver/cleanup"
	driverinstall "github.com/draios/sysdig/userspace/driver-loader/cmd/driver/install"
	"github.com/draios/sysdig/userspace/driver-loader/internal/config"
...
func NewDriverCmd(ctx context.Context, opt *options.Common) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:                 "Interact with scap driver",
		Long:                  `Interact with scap driver.`,
...
func loadDriverVersion() string {
	...
	paths, _ := filepath.Glob("/usr/src/scap-*")
	for _, path := range paths {
		...
		drvVer := strings.TrimPrefix(filepath.Base(path), "scap-")
